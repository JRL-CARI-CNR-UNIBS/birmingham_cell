...
class GenericRealForceFakeEnv(gym.Env):
    ...
    def _get_obs(self) -> Dict[str, np.array]:
        observation = np.concatenate([np.array(self.param_values),np.array(self.current_grasp_pos),np.array(self.current_insert_pos),np.array([self.grasp_zone]),np.array(self.grasp_forces),np.array(self.insertion_forces),np.array(self.object_info)])
        return observation
    ...
    def _get_reward(self) -> float:
        ...
        if (grasp_zone == 'successfully_grasp'):
            ...
            self.insertion_forces = [(self.current_insert_pos[0] - self.correct_insert_pos[0]) * 10, -(self.current_insert_pos[1] - self.correct_insert_pos[1]) * 10]
