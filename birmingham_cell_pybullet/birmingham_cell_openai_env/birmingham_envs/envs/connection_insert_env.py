...
class ConnectionInsertEnv(gym.Env):
    ...
    def __init__(
        ...
        self.insert_param_name = '/exec_params/actions/insertion/skills/move_to_insertion/relative_position'
        ...
        self.init_par_val      = np.array([ 0.00, 0.00])
        ...
        new_tfs = []
        for tf_ in current_tfs:
            new_tf = copy.copy(tf_)
            if new_tf['name'] == 'can_grasp':
                new_tf['position'][0] = 0.0
                new_tf['position'][1] = 0.0
    ...
    def _get_obs(self) -> Dict[str, np.array]:
        observation = np.concatenate([self.param_values,self.current_insert_pos,self.insert_forces/10000])
        return observation
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None
        ...
        rospy.set_param(self.insert_param_name, [float(self.param_values[0]),float(self.param_values[1]),0.0])
    ...
    def step(self, action: np.array) -> Tuple[Dict[str, np.array], float, bool, bool, Dict[str, Any]]:
        ...
        rospy.set_param(self.insert_param_name, [float(self.param_values[0]),float(self.param_values[1]),0.0])
        self.current_insert_pos[0] = self.initial_insert_pos[0] + self.param_values[0]
        self.current_insert_pos[1] = self.initial_insert_pos[1] - self.param_values[1]
        ...
        print('Current insert pose: ' + str(self.current_insert_pos))
    ...
    def _get_reward(self) -> float:
        reward = 1 - self._distance(self.obj_pos[0:2], self.tar_pos[0:2])       
        return reward
