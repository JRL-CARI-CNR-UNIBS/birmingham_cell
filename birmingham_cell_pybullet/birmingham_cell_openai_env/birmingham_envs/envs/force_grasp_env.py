...
class ForceGraspEnv(gym.Env):
    ...
    def __init__(
        ...
        self.param_upper_bound = np.array([0.05,0.05])
        self.param_lower_bound = np.array([-0.05,-0.05])
        self.init_par_val      = np.array([0.0,0.0])
...
# //
    ...
    def _get_obs(self) -> Dict[str, np.array]:
        observation = np.concatenate([self.param_values,self.current_grasp_pos,self.grasp_forces/1000])
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None
        ...
        self.epoch_steps = 0
        pose_err_low_limit = [-0.019, -0.009]
        pose_err_high_limit = [0.019, 0.009]

        obj_pos_error = np.ndarray.tolist(self.np_random.uniform(pose_err_low_limit, pose_err_high_limit))
        ...
        # print('current_to_correct_distance: ' + str(current_to_correct_grasp_distance))
        # print(self.grasp_forces)
        # print('forces lenght: ' + str(len(self.grasp_forces)))
        observation = self._get_obs()
        info = {"is_success": False}
        return observation, info
    
    def _is_success(self) -> np.array:
        if (self._distance(self.current_grasp_pos,self.correct_grasp_pos) < self.distance_threshold):
            success = True
    ...
    def step(self, action: np.array) -> Tuple[Dict[str, np.array], float, bool, bool, Dict[str, Any]]:
        ...
        current_to_correct_distance = self.current_grasp_pos - self.correct_grasp_pos
        calculed_grasp_forces = self._generate_forces(current_to_correct_distance[0],current_to_correct_distance[1],self.grasp_pose_to_forces)
        self.grasp_forces = np.concatenate([calculed_grasp_forces['fx'],
                                            calculed_grasp_forces['fy'],
                                            calculed_grasp_forces['fz'],
                                            calculed_grasp_forces['tx'],
    ...
    def _get_reward(self) -> float:
        if self.in_grasp_area:
            reward = 1 - (self._distance(self.current_grasp_pos, self.correct_grasp_pos) * 10)
        #     print('In')
        #     print(reward)
        else:
            reward = 0 - (self._distance(self.current_grasp_pos, self.theoretical_correct_grasp_pos) * 10)
            # print('Out')
            # print(reward)
        return reward
