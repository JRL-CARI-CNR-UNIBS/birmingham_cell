...
class ConnectionGraspEnv(gym.Env):
    ...
    def _get_obs(self) -> Dict[str, np.array]:
        observation = np.concatenate([self.param_values,self.current_grasp_pos,self.grasp_forces/10000])
        return observation
    ...
    def _is_success(self) -> np.array:
        distance = np.linalg.norm(self.current_grasp_pos[0:2])
    ...
    def step(self, action: np.array) -> Tuple[Dict[str, np.array], float, bool, bool, Dict[str, Any]]:
        ...
        print('Current grasp pose: ' + str(self.current_grasp_pos))
    ...
    def _get_reward(self) -> float:
        reward = 1 - np.linalg.norm(self.current_grasp_pos[0:2])       
        return reward
