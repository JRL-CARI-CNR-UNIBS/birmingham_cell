...
class personal_optimizer:
    def __init__(self, package_path, tree_name, data_name, iteration):
        ...
        self.tree_folder_path = package_path + '/config/trees'

        if (self.data_name + '.yaml') in os.listdir(self.data_path):
            if (self.data_name + '_exec_params.yaml') in os.listdir(self.data_path):
                print('Load params')
                os.system('rosparam load ' + self.data_path + '/' + self.data_name + '.yaml /' + self.data_name)
                print('History params loaded')
                os.system('rosparam load ' + self.data_path + '/' + self.data_name + '_exec_params.yaml /exec_params')
                print('exec params loaded')
            else:
                print('No exec params file. It remain the start one.')
        ...
        for param_name in all_param_names:
            if (param_name.find('exec_params/actions') != -1):
                self.param_names.append(param_name)
                param_name = param_name.replace('exec_params/actions/', '')
...
#        print(self.tree_folder_path)
        ...
        for param_name in all_param_names:
            if (param_name.find('total_reward') != -1):
                ...
                reward_names.append(param_name.replace('exec_params/actions/', ''))
...
#                print(param_name + ': ' + str(type(param_value)))
    ...
    def run_optimization(self): 
        ...
        for i in range(missing_iterations):
            ...
            os.system('rosparam dump ' + self.data_path + '/' + self.data_name + '_exec_params.yaml /exec_params')
